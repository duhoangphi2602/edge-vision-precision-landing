...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `TRN_001` | 640px | 16 | 30.1% | 26.1% | Baseline model from Day 03. |
| **`TRN_002`** | **960px** | **8** | **37.4%** | **35.4%** | Required Batch=8 to prevent GPU OOM. |

## Key Findings & Conclusion
1. **Massive Accuracy Gain:** Increasing the resolution from 640px to 960px resulted in a **+7.3% absolute increase in mAP50**.
2. **Hypothesis Confirmed:** This proves definitively that the low accuracy in `TRN_001` was NOT due to a weak model architecture (YOLO26n is capable), but because the 640px resolution heavily degraded the visual features of tiny objects (like distant drones or cars), making them impossible to detect.
